...
function prepare_certs {
  ...
  if [[ -z ${CA_PASSWORD} ]]; then
    export CA_PASSWORD=$(fetch_secret "/repo/${ENVIRONMENT}/user-input/ca-password")
  fi
  if [[  ! -f ${PKI_DIR}/certs/ca.crt ]]; then
    mkdir -p ${PKI_DIR}/certs
    fetch_secret "/repo/${ENVIRONMENT}/user-input/ca-crt" > "${PKI_DIR}/certs/ca.crt"
  fi
  if [[ ! -f ${PKI_DIR}/private/ca.key ]]; then
    mkdir -p ${PKI_DIR}/private
    fetch_secret "/repo/${ENVIRONMENT}/user-input/ca-key" > "${PKI_DIR}/private/ca.key"
  fi
